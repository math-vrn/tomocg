...
class Solver(object):
    ...
    def line_search(self, minf, gamma, u, d):
        while(minf(u)-minf(u+gamma*d) < 0 and gamma > 1e-16):
            gamma *= 0.5
    ...
    def cg_tomo0(self, data, init, titer):
        # minimization functional
        def minf(Ru):
            return cp.linalg.norm(Ru-data)**2
        u = init.copy()
        gamma = 2  # init gamma as a large value
        for i in range(titer):
            Ru = self.fwd_tomo_batch(u)            
            grad = self.adj_tomo_batch(Ru-data)                
            # Dai-Yuan direction
            d = -grad
            # if i == 0:
            #     d = -grad
            ...
            gamma = self.line_search(
                minf, gamma, Ru, self.fwd_tomo_batch(d))
            # update step
            u = u + gamma*d
            print(i,gamma,minf(Ru))
        return u
