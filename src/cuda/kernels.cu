...
void __global__ gather(float2 *g, float2 *f, float *x, float *y, int M,
  ...
  for (int i1 = 0; i1 < 2 * M + 1; i1++) {
    ...
    for (int i0 = 0; i0 < 2 * M + 1; i0++) {
      ...
      float w = PI / (sqrtf(mu * mu)) *
                __expf(-PI * PI / mu * (w0 * w0) - PI * PI / mu * (w1 * w1));
      g0.x += w * f[N + M + ell0 + (2 * N + 2 * M) * (N + M + ell1) +
                    tz * (2 * N + 2 * M) * (2 * N + 2 * M)].x;
      g0.y += w * f[N + M + ell0 + (2 * N + 2 * M) * (N + M + ell1) +
                    tz * (2 * N + 2 * M) * (2 * N + 2 * M)].y;
    }
  }
...
void __global__ scatter(float2 *f, float2 *g, float *x, float *y, int M,
                        float mu, int N, int Ntheta, int Nz) {
  int tx = blockDim.x * blockIdx.x + threadIdx.x;
  int ty = blockDim.y * blockIdx.y + threadIdx.y;
  int tz = blockDim.z * blockIdx.z + threadIdx.z;
  ...
  for (int i1 = 0; i1 < 2 * M + 1; i1++) {
    ...
    for (int i0 = 0; i0 < 2 * M + 1; i0++) {
      ...
      float w = PI / (sqrtf(mu * mu)) *
                __expf(-PI * PI / mu * (w0 * w0) - PI * PI / mu * (w1 * w1));
      float *fx = &(f[N + M + ell0 + (2 * N + 2 * M) * (N + M + ell1) +
                      tz * (2 * N + 2 * M) * (2 * N + 2 * M)].x);
      float *fy = &(f[N + M + ell0 + (2 * N + 2 * M) * (N + M + ell1) +
                      tz * (2 * N + 2 * M) * (2 * N + 2 * M)].y);
      atomicAdd(fx, w * g0.x);
      atomicAdd(fy, w * g0.y);
    }
